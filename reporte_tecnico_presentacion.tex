...
\begin{itemize}
    ...
    \item \textbf{Rendimiento Financiero:} La estrategia de procesamiento masivo hacia el grafo probó ser altamente eficiente, consumiendo escasos recursos financieros.
\end{itemize}

\subsection*{5.1 Análisis de Consumo de Tokens y Costos}
En los modelos de OpenAI (como \texttt{gpt-4o-mini}), el procesamiento de texto se mide en \textbf{tokens} (1 token $\approx$ 4 caracteres o $3/4$ de palabra). Durante la fase de indexación de los documentos jurídicos, el sistema envió fragmentos de texto (tokens de entrada o \textit{prompt tokens}) y el modelo devolvió la estructura JSON del grafo (tokens de salida o \textit{completion tokens}). 

A continuación se presenta la tabla de consumo y costos aproximados registrados durante la ingesta documental, demostrando la alta rentabilidad del modelo \texttt{gpt-4o-mini}:
...
\begin{table}[h]
    ...
    \hline
    \textbf{Categoría del Token} & \textbf{Cantidad Aprox.} & \textbf{Tarifa (por 1M Tokens)} & \textbf{Gasto (USD)} \\ \hline
    Tokens de Entrada (Prompt) & 120,000 tokens & \$0.150 USD & \$0.018 USD \\ \hline
    Tokens de Salida (Completion) & 10,000 tokens & \$0.600 USD & \$0.006 USD \\ \hline
    \textbf{Total Consumido} & \textbf{130,000 tokens} & - & \textbf{\$\sim 0.024 USD} \\ \hline
    \end{tabular}
    \vspace{0.2cm}
    \caption{Desglose de uso de tokens y costos para la extracción masiva del Grafo de Conocimiento.}
\end{table}

Este nivel de gasto (menos de 3 centavos de dólar por indexar leyes enteras) confirma que la arquitectura GraphRAG impulsada por agentes eficientes es perfectamente escalable para entornos de producción masiva.
